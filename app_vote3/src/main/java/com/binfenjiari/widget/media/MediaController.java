...
public class MediaController {
    ...
    /**
     * 一个控制接口，通常是媒体控制器需要控制的对象，比如视频播放控件
     */
    ...
    public MediaController(String path, View controlView)
    {
        mPath = path;
        mMediaHandler = new MediaHandler();
        mAppContext = controlView.getContext().getApplicationContext();
        initControlView(controlView);
    }

    private void initControlView(View controlView) {
        mVideoView = Views.find(controlView, R.id.videoView);
        ...
        mControlView = Views.find(controlView, R.id.media_controller);
        mControlView.setVisibility(View.GONE);
        mPauseView = Views.find(controlView, R.id.media_pause_start);
        mCurTime = Views.find(controlView, R.id.tv_time_curr);
        mEndTime = Views.find(controlView, R.id.tv_time_end);
        mSb = Views.find(controlView, R.id.sb);
        mSb.setOnSeekBarChangeListener(mSeekBarChangeListener);
        mFullScreen = Views.find(controlView, R.id.fullScreen);
    ...
    private void togglePauseStartView(int visibility,boolean forcePlay) {
        ...
        if (mPauseView.isShown()) {
            mPauseView.setImageResource(
                    ...
                    ? R.drawable.audio_pause : R.drawable.audio_start);
        }
    }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_SHOW_PROGRESS:
                    if (mControlView.isShown() && !mIsDragging && mControl != null &&
                        mControl.getCurrentState() == VideoView.State.STARTED && canSetProgress())
                    {
...
//        }
    ...
    public boolean playVideo(int preSeekToPosition) {
        ...
        if (canStart()) {
            ...
            Msgs.shortToast(mAppContext, "很抱歉,视频无法播放");
        }
    ...
    public void reallyPlayNewVideo(int preSeekToPosition) {
        ...
        // TODO 解耦
        //停止旧的视频
        //MediaHelper.stopPlayingVideo(mIsVideoList,);

        if (TextUtils.isEmpty(mPath)) {
            Msgs.shortToast(mAppContext, "视频播放地址错误");
            return;
        }
    ...
    public void stopPlayingVideo() {
        MediaHelper.stopPlayingVideo(mVideoView);
    }
            ...
            MediaPlayer.OnBufferingUpdateListener,MediaPlayer.OnInfoListener
    {
        ...
        public boolean onError(MediaPlayer mp, int what, int extra) {
            ...
            Msgs.shortToast(mControlView.getContext(),"视频播放失败");
...
//                mProgressBar.setVisibility(UserView.GONE);
//            }
        }
