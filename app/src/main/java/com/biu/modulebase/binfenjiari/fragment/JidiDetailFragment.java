...
public class JidiDetailFragment extends BaseFragment {
    ...
    public static JidiDetailFragment newInstance(int position,String id) {
        Bundle args = new Bundle();
        args.putInt(Constant.KEY_POSITION, position);
        args.putString(Constant.KEY_ID, id);
        JidiDetailFragment fragment = new JidiDetailFragment();
    ...
    {
        ViewGroup rootView= (ViewGroup) inflater.inflate(R.layout.fragment_jidi_detail,container,false);
    ...
    protected void initView(View rootView) {
        ...
        args.put(Constant.KEY_MODEL, Constant.MODEL_PLACE_COMMENT_DETAIL);
        args.put(Constant.KEY_ACTION, Constant.ACTION_PLACE_COMMENT_DETAIL);
        //回复主评论或回复 的 action 和 model
        args.put(Constant.KEY_MODEL_COMMENT_DETAIL,Constant.MODEL_PLACE_COMMENT_DETAIL);
        args.put(Constant.KEY_ACTION_COMMENT_DETAIL,Constant.ACTION_PLACE_COMMENT_REPLY);

        //删除评论 的 action 和 model
        args.put(Constant.KEY_MODEL_DELETE_COMMENT_COMMENT_DETAIL,Constant.MODEL_PLACE_COMMENT);
        args.put(Constant.KEY_ACTION_DELETE_COMMENT_COMMENT_DETAIL,Constant
                .ACTION_PLACE_COMMENT_DELETE);

        //删除回复 的 action 和 model
        args.put(Constant.KEY_MODEL_DELETE_REPLY_COMMENT_DETAIL,Constant.MODEL_PLACE_COMMENT_DETAIL);
        args.put(Constant.KEY_ACTION_DELETE_REPLY_COMMENT_DETAIL,Constant.ACTION_PLACE_COMMENT_REPLY_DELETE);

        //删除回复或评论的 的字段 名
        args.put(Constant.KEY_NAME_ARGS, "baseId");
        //删除回复或评论的 的字段 值
        args.put(Constant.KET_VALUE_ARGS, mId);

        //评论举报类型
        args.put(Constant.KEY_REPORT_TYPE,Constant.REPORT_PLACE_COMMENT);

        //评论基础参数设置
        JSONObject params = OtherUtil.getJSONObject(getActivity(), Constant.MODEL_PLACE_COMMENT,
                Constant.ACTION_PLACE_COMMENT_LIST, false);
        JSONUtil.put(params, "id", mId);
        //评论处理类(加载，回复，删除 评论)
        mCommentLoader = new CommentLoader(CommentLoader.TYPE_LIST, this, params, args);

        final CommentAvailableAdapter adapter=new CommentAvailableAdapter(this,mCommentLoader, args ,new
                ArrayList<ParentListItem>(),
                R.layout.header_jidi_detail, new ViewHolderCallbacks() {
            @Override
                    ...
                    int adapterPosition, int parentPosition, int parentAdapterPosition)
            {
                ...
                if (i == R.id.tv_location) {
                    CommentAvailableAdapter adapter =
                            (CommentAvailableAdapter) mRecyclerView.getAdapter();
                    JidiHeaderParentItem jidiHeader =
                            (JidiHeaderParentItem) adapter.getHeader(parentPosition);
                    Intent intent = new Intent(getActivity(), MapActivity.class);
                    intent.putExtra(Constant.KEY_LOCATION_LATITUDE, jidiHeader.getLatitude());
                    intent.putExtra(Constant.KEY_LOCATION_LONGITUDE, jidiHeader.getLongitude());
                    intent.putExtra(Constant.KEY_MAP_TARGET, jidiHeader.getAddress());
            ...
            public void bindData(RecyclerView.ViewHolder viewHolder, Object data) {
                if (data==null||!(data instanceof JidiHeaderParentItem)) return;

                JidiHeaderParentItem item= (JidiHeaderParentItem) data;
                ...
                tv_location.setText(getString(R.string.address2,item.getAddress(), DoubleUtil.round(item.getDistance(),2)));

                TextView tv_activity_number= (TextView) holder.getView(R.id.tv_activity_number);
                tv_activity_number.setText(getString(R.string.activity_number,item.getActivity_number()));
                ...
                if (mHeaderHandler!=null) {
                    mHeaderHandler.createBindLabelViews((FlowLayout) holder.getView(R.id
                            .layout_label),item.getLabel());
        ...
        rootView.findViewById(R.id.reservation).setOnClickListener(this);
...
//        OtherUtil.getLocation(getActivity(),location);
        ...
        JSONObject params= OtherUtil.getJSONObject(mBaseActivity,Constant.MODEL_PLACE,Constant
        .ACTION_PLACE_DETAIL,false);
...
//                visibleLoading();
            ...
            public void onSuccess(String mainJsonString,JSONObject mainJsonObject,JSONObject rootJsonObject) {
                ...
                jidiHeader=JSONUtil.fromJson(mainJsonString,
                        JidiHeaderParentItem.class);
                oldLikeStatus =jidiHeader.getLike_status();
                //头部数据加载完成设置给评论类
                mCommentLoader.addHeaderData(jidiHeader);
            }
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.reservation) {
            if (Utils.isInteger(jidiHeader.getActivity_number()) == 0) {
                showTost("该基地未举办活动", 1);
                return;
            }
            ...
            JSONObject params = OtherUtil.getJSONObject(getActivity(),
                    Constant.MODEL_PLACE_COMMENT, Constant.ACTION_PLACE_COMMENT, true);
            JSONUtil.put(params, "id", mId);
            mCommentLoader.doComment(params, -1);


        } else if (i == R.id.like) {
            JSONObject params2 = OtherUtil.getJSONObject(getActivity(), Constant.MODEL_PLACE, Constant
                    .ACTION_PLACE_LIKE, true);
            ...
            OtherUtil.collect(this, mCollectBox, mId, Constant.MODEL_PLACE, Constant
                    .ACTION_PLACE_COLLECT);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (i == R.id.action_more) {
            OtherUtil.showMoreOperate(JidiDetailFragment.this, mId, jidiHeader.getName(), jidiHeader.getName(), null, Constant.SHARE_PLACE,
                    Constant.REPORT_PLACE, null, false, null, -1, null);
