...
public class Utils {
     ...
     * 将double保留1位小数点
     **/
    public static double DecimalPoint(Object value) {
        double v;
     ...
     * 设置 排行版的名次
     */
    public static void setRankListState(TextView rankOrder, int order) {

        order=order+1;
        if (order == 1) {
            rankOrder.setBackgroundResource(R.mipmap.icon_number_one);
            rankOrder.setText("");
        } else if (order == 2) {
            rankOrder.setBackgroundResource(R.mipmap.icon_number_two);
            rankOrder.setText("");
        } else if(order == 3){
            rankOrder.setBackgroundResource(R.mipmap.icon_number_three);
            rankOrder.setText("");
        }else{
            rankOrder.setBackgroundResource(0);
            rankOrder.setText(order+"");
        }
     ...
     * 设置性别图标  1男 2女 0不展示
     */
    public static void setSexIconState(Context context, TextView name, int status) {

        if (status == 1) {
            Drawable drawableMan = context.getResources().getDrawable(R.mipmap.icon_sex_man);
            drawableMan.setBounds(0, 0, drawableMan.getMinimumWidth(), drawableMan.getMinimumHeight());
            name.setCompoundDrawables(null, null, drawableMan, null);

//            if(name.getText().length()>7){
//                name.setCompoundDrawablePadding(Utils.dp2px(context,-7));
//            }else{
//                name.setCompoundDrawablePadding(Utils.dp2px(context,-4));
//            }
        } else if (status == 2) {
            Drawable drawableWoman = context.getResources().getDrawable(R.mipmap.icon_sex_woman);
            drawableWoman.setBounds(0, 0, drawableWoman.getMinimumWidth(), drawableWoman.getMinimumHeight());
            name.setCompoundDrawables(null, null, drawableWoman, null);
...
//        return m.matches();
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (s.toString().startsWith("0") && s.toString().trim().length() > 1) {
                    if (!s.toString().substring(1, 2).equals(".")) {
                        editText.setText(
                                Utils.isDouble(s.subSequence(0, s.length()).toString()) + "");
    ...
    public static String checkDouble(String num) {
        return (num == null || !Utils.isDoubleNumeric(num)) ? "0" : num;
    }

    public static String checkFloat(String num) {
        return (num == null || !Utils.isFloatNumeric(num)) ? "0" : num;
    }
     ...
     * 渠道标志为： 1，andriod（a）
     *
     * 识别符来源标志： 1， DEVICE_ID 2， IMEI（imei）； 3， 序列号（sn）； 4，
    ...
    public static String getDeviceID(Context context) {
        ...
        try {
            ...
            if (ANDROID_ID != null && ANDROID_ID.trim().length() != 0) {
    ...
    public static String encodeM(String password) {
