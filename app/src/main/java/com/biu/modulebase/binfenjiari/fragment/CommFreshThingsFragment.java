...
public class CommFreshThingsFragment extends BaseFragment {
    ...
    protected void initView(View rootView) {
        ...
        mRefreshLayout.setSwipeRefreshListener(new LSwipeRefreshLayout.SwipeRefreshListener() {
            ...
            public void onLoadMore() {
                pageNum++;
                getNewsList(Constant.LIST_LOAD_MORE);
            }
        });
        ...
        BaseAdapter adapter = new BaseAdapter(getActivity()) {
            ...
            public BaseViewHolder getViewHolder(ViewGroup parent, int viewType) {
                BaseViewHolder holder = new BaseViewHolder(LayoutInflater.from(getActivity())
                        .inflate(R.layout.item_fresh_things, parent, false), new BaseViewHolder
                        .Callbacks2() {
                    @Override
                    public void bind(BaseViewHolder holder, Object data) {
                        final NewsVO bean = (NewsVO) data;
                    ...
                    public void onItemClick(BaseViewHolder holder, View view, final int position) {
                        final NewsVO bean = (NewsVO) getData().get(position);
                        int i = view.getId();
                        if (i == R.id.ib_more) {
                            OtherUtil.showMoreOperate(CommFreshThingsFragment.this, bean.getId
                                    (), bean.getContent(), bean.getContent(), bean.getUser_id(), Constant.SHARE_NEWS, Constant.REPORT_NEWS, Constant
                                    .DELETE_NEWS, true, (BaseAdapter) mRecyclerView.getAdapter
                                    (), position, null);

                        } else {
                            Intent intent = new Intent(getActivity(), CommFreshDetailActivity.class);
    ...
    public void loadData() {
        ...
        pageNum = 1;
        getNewsList(Constant.LIST_REFRESH);

    }

    private void getNewsList(final int tag) {
        JSONObject params = new JSONObject();
        try {
            params.put("model", Constant.MODEL_NEWS);
            params.put("action", Constant.ACTION_NEWS_HOME);
        ...
        jsonRequest(false, params, Constant.SERVERURL, getClass().getSimpleName().toString(), new ContextRequestCallBack() {
            ...
            public void onSuccess(String jsonString) {
                try {
                    ...
                    JSONArray array = JSONUtil.getJSONArray(result, "newsList");
                    ArrayList<NewsVO> list = JSONUtil.fromJson(array.toString(), new TypeToken<List<NewsVO>>() {
                    }.getType());
            ...
            public void onCodeError(int key, String message) {
                if (key == 3) {
                    refreshList(tag, new ArrayList<NewsVO>());
                } else {
                    showTost(message, 1);
                }
            }
    ...
    private void refreshList(int tag, ArrayList<NewsVO> datas) {
