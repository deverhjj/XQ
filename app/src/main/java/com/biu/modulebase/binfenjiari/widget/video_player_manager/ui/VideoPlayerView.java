package com.biu.modulebase.binfenjiari.widget.video_player_manager.ui;
...
import com.biu.modulebase.binfenjiari.widget.video_player_manager.utils.HandlerThreadExtension;
...
public class VideoPlayerView extends ScalableTextureView
    ...
    public MediaPlayerWrapper.State getCurrentState() {
        synchronized (mReadyForPlaybackIndicator) {
            return mMediaPlayer!=null? mMediaPlayer.getCurrentState():MediaPlayerWrapper.State.IDLE;
        }
    }
...
//        if (SHOW_LOGS) Logger.v(TAG, "onSurfaceTextureUpdated, mIsVideoStartedCalled " + mIsVideoStartedCalled.get() + ", mMediaPlayer.getState() " + mMediaPlayer.getState());
    ...
    private MediaPlayerWrapper.VideoStateListener mOuterVideoStateListener;

    public void setVideoStateListener(MediaPlayerWrapper.VideoStateListener listener){
        mOuterVideoStateListener=listener;
    }
    @Override
    public void onVideoPlayTimeChanged(int positionInMilliseconds) {
        if (mOuterVideoStateListener!=null) {
            mOuterVideoStateListener.onVideoPlayTimeChanged(positionInMilliseconds);
        }
    }
        ...
        synchronized (mReadyForPlaybackIndicator){
            if(mReadyForPlaybackIndicator.isReadyForPlayback()){
                ...
                if(!mReadyForPlaybackIndicator.isFailedToPrepareUiForPlayback()){
                    ...
                    if (SHOW_LOGS) Logger.w(TAG, "start, movie is not ready. Av size will not become available");
                }
            }
        }
        if (SHOW_LOGS) Logger.v(TAG, "<< start");
    ...
    public MediaPlayerWrapper.State getState() {
        return mMediaPlayer == null ? MediaPlayerWrapper.State.IDLE
                : mMediaPlayer.getCurrentState();
    }
